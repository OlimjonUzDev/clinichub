...
class PrescriptionsViewSetTestCase(APITestCase):
    """PrescriptionViewSet uchun ruxsat va CRUD xatti-harakatlarini tekshiruvchi testlar to'plami."""

    def setUp(self):
        """Testlar uchun klinika, shifokorlar, bemorlar, uchrashuvlar va namunaviy retsept yaratadi."""
    ...
    def test_owner_doctor_can_create_prescription_with_items(self):
        """Shifokor o'z uchrashuvi uchun dori-darmon bandlari bilan retsept yarata olishini tekshiradi."""
    ...
    def test_prescription_list_filtered_to_own_patient(self):
        """Bemor retseptlar ro'yxatini so'raganda 200 status qaytishini tekshiradi (ro'yxat o'ziga filtrlanadi)."""
        url = reverse('prescription-list')
    ...
    def test_prescription_list_filtered_to_own_doctor(self):
        """Shifokor retseptlar ro'yxatini so'raganda 200 status qaytishini tekshiradi (ro'yxat o'ziga filtrlanadi)."""
        url = reverse('prescription-list')
        self.client.force_authenticate(self.doctor_user)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_other_doctor_cannot_retrieve_prescription(self):
        """Boshqa shifokor o'ziga tegishli bo'lmagan retseptni ochishga urinsa 404 qaytishini tekshiradi."""
    ...
    def test_patient_cannot_update_prescription(self):
        """Bemor retseptni yangilashga urinsa 403 (ruxsat yo'q) qaytishini tekshiradi."""
        url = reverse('prescription-detail', args=[self.prescription.pk])
        self.client.force_authenticate(self.owner_user)
    ...
    def test_update_with_invalid_item_data_returns_400_not_500(self):
        """Noto'g'ri dori-darmon bandi bilan yangilashda 500 emas, 400 xatolik qaytishini tekshiradi."""
        url = reverse('prescription-detail', args=[self.prescription.pk])
        self.client.force_authenticate(self.doctor_user)
    ...
    def test_update_replaces_items_when_items_provided(self):
        """Yangilashda "items" berilsa, eski bandlar yangilari bilan almashtirilishini tekshiradi."""
        url = reverse('prescription-detail', args=[self.prescription.pk])
        self.client.force_authenticate(self.doctor_user)
    ...
    def test_update_without_items_keeps_existing_items(self):
        """Yangilashda "items" berilmasa, mavjud dori-darmon bandlari o'zgarishsiz qolishini tekshiradi."""
        url = reverse('prescription-detail', args=[self.prescription.pk])
        self.client.force_authenticate(self.doctor_user)
